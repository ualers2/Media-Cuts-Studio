...
users_ref = db.reference("Users_Control_Panel", app=app_instance)
schedules_ref_root = db.reference("user_schedules", app=app_instance)
VIDEO_BASE_DIR = os.path.join(os.path.dirname(__file__), 'videos')
firebase_db_status = db.reference('status', app=app_instance)
...
@app.route('/')
def index():
    return jsonify({"message": "#1 Landingpage API Media Cuts Studio funcionando!"})
...
def metrics_channel_youtube():
    ...
    try:
        ref = db.reference(f'metrics/youtube/{key}', app=app_instance)
        ref.set(videos)
    except Exception as e:
...
def save_lead():
    ...
    ref = db.reference("lead-form", app=app_instance)
...
def endpoint_create_login():
    ...
    try:
        ...
        all_users_ref = db.reference('Users_Control_Panel', app=app_instance)
...
def new_account_tiktok():
    ...
    ref_tasks = db.reference(f'users_account_cookies/{api_key}', app=app_instance)
...
def new_account_instagram():
    ...
    ref_tasks = db.reference(f'users_account_instagram/{api_key}', app=app_instance)
...
def get_active_accounts():
    try:
        config = request.get_json()
        api_key = config['api_key']
        ref1 = db.reference(f'users_account_cookies/{api_key}', app=app_instance)
        snapshot1 = ref1.get() or {}
        ref2 = db.reference(f'users_account_instagram/{api_key}', app=app_instance)
...
def get_user_config(user_id):
    ...
    try:
        ref = db.reference(f'user_configs/{fixx_email}', app=app_instance)
        config = ref.get()
        if config is None:
...
def tarefas_config(user_id):
    fixx_email = user_id.replace(".", "_")
    try:
        ref = db.reference(f'user_tasks/{fixx_email}', app=app_instance)
        tasks = ref.get()
        if tasks is None:
...
def user_account_instagram():
    ...
    for tentativa in range(4):
        ...
        snapshot = db.reference(f'users_account_instagram/{api_key}', app=app_instance).get() or {}
...
def user_account():
    ...
    for tentativa in range(4):
        logger.info(f"Tentativa {tentativa + 1} de 2 para localizar usuário")
        snapshot = db.reference(f'users_account_cookies/{api_key}', app=app_instance).get() or {}
...
def count_tiktok_accounts(api_key):
    ...
    """
    try:
        ref = db.reference(f'users_account_cookies/{api_key}', app=app_instance)
        snapshot = ref.get() or {}
        count = 0
