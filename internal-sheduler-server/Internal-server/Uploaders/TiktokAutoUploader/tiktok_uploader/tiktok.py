...
file_handler = logging.FileHandler(os.path.join(path_logger, 'TiktokUploader_2.log'))
...
# Normaliza cookies
# -------------------
def normalize_cookies(cookies_input):
	if isinstance(cookies_input, str):
		try:
			cookies = json.loads(cookies_input)
		except json.JSONDecodeError:
			cookies = ast.literal_eval(cookies_input)
	else:
		cookies = cookies_input

	# Se for dict único, vira lista
	if isinstance(cookies, dict):
		cookies = [cookies]

	# Se for lista de strings JSON, converte cada item
	normalized = []
		...
		if isinstance(c, str):
			try:
				...
				try:
					...
					logger.error(f"Falha ao parsear item {idx} de cookies: ({e})")
					continue
		if not isinstance(c, dict):
			logger.error(f"Item de cookies não é dict: {c}")
			continue
		normalized.append(c)

	return normalized

def upload_video(session_user_cookie, video, title, schedule_time=0, allow_comment=1, allow_duet=0, allow_stitch=0, visibility_type=0, brand_organic_type=0, branded_content_type=0, ai_label=0, proxy=None, webhook=True, WEBHOOK_URL=None, api_key=None):
	"""

Tamanho e duração
Tamanho máximo: 30 GB
Duração máximo do vídeo: 60 minutos.

Formatos de arquivo
Recomendado: “.mp4”. São suportados outros formatos principais.

Resoluções de vídeo
Alta resolução recomendada: 1080p, 1440p e 4K.

Proporções de apresentação
Recomendado: 16:9 para paisagem, 9:16 para vertical.
	...
	if isinstance(session_user_cookie, str):
		try:
			cookies = ast.literal_eval(session_user_cookie)
		except Exception as e:
			logger.error(f"Erro ao converter cookies: {e}")
			try:
				cookies = json.loads(session_user_cookie)
			except Exception as e:
				logger.error(f"Erro ao converter cookies: {e}")

	else:
		cookies = session_user_cookie

	cookies = normalize_cookies(session_user_cookie)
	logger.info(f"cookies normalizados ({len(cookies)}): {cookies}")
	...
	logger.info(f"session_id extraído: {session_id}")
	logger.info(f"dc_id extraído: {dc_id}")
	...
	logger.info(f"Tiktok Datacenter Assigned: {dc_id}")
	if webhook == True:
		send_to_webhook(WEBHOOK_URL, api_key, "notification", f"🚀 User successfully logged in", "yellow")
		send_to_webhook(WEBHOOK_URL, api_key, "info", f"🚀 User successfully logged in", "yellow")

	logger.info("Uploading video...")
	if webhook == True:
		# send_to_webhook(WEBHOOK_URL, api_key, "notification", f"🚀 Uploading video...", "yellow")
		send_to_webhook(WEBHOOK_URL, api_key, "info", f"🚀 Uploading video...", "yellow")

	# Parameter validation,
	if schedule_time and (schedule_time > 864000 or schedule_time < 900):
		logger.info("[-] Cannot schedule video in more than 10 days or less than 20 minutes")
		return False
	if len(title) > 2200:
		logger.info("[-] The title has to be less than 2200 characters")
		return False
	if schedule_time != 0 and visibility_type == 1:
		logger.info("[-] Private videos cannot be uploaded with schedule")
		return False
	...
	while True:
		...
		js_path = os.path.join(base_dir, "tiktok-signature", "browser.js")
		...
		if not assertSuccess(url, r):
			logger.info("[-] Published failed, try later again")
			printError(url, r)
			return False, None, None, None

		if r.json()["status_code"] == 0:
			logger.info(f"Published successfully {'| Scheduled for ' + str(schedule_time) if schedule_time else ''} {video_id}, {upload_id}, {creation_id}")
			if webhook == True:
				send_to_webhook(WEBHOOK_URL, api_key, "notification", f"🚀 Published successfully {'| Scheduled for ' + str(schedule_time) if schedule_time else ''}", "yellow")
				send_to_webhook(WEBHOOK_URL, api_key, "info", f"🚀 Published successfully {'| Scheduled for ' + str(schedule_time) if schedule_time else ''}", "yellow")
			uploaded = True
			return True, video_id, upload_id, creation_id
		else:
			logger.info("[-] Publish failed to Tiktok, trying again...")
			printError(url, r)
			
		return False, None, None, None
		#
		# logger.info("[-] Could not upload video")
		# return False, None, None, None
...
def send_to_webhook(WEBHOOK_URL, user, type, message, cor):
