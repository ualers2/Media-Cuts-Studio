  ...
  async sign(link) {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        lastError = error;
        console.warn(`Erro na geração de assinatura (tentativa ${attempt + 1}):`, error);
        
        // Se for erro de sign indefinido, reinicializa contexto
        await this.close();
        await this.init();
        attempt++;
      }
    ...
    // Após tentativas, lança o último erro
    throw lastError;
  }
