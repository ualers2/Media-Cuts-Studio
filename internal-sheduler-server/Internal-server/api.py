...
shorts_ref = db.reference("shorts", app=app1)
...
def schedule_post():
    ...
    if 'youtube' in post_social_networks and 'tiktok' in post_social_networks:
        ...
        canal_data_tiktok = db.reference(f"canais/{canal_id_tiktok}", app=app1).get()
        if not canal_data_tiktok:
            return jsonify({"error": f"Canal com ID {canal_id_tiktok} não encontrado."}), 404
        
        canal_data_youtube = db.reference(f"canais/{canal_id}", app=app1).get()
    ...
    # ==== Somente YouTube ====
        ...
        canal_data = db.reference(f"canais/{canal_id}", app=app1).get()
        if not canal_data:
            return jsonify({"error": f"Canal com ID {canal_id} não encontrado."}), 404
...
#     if batch_id not in upload_results:
#         return jsonify({
#             "status": "processing",
#             "message": "Upload ainda em andamento..."
#         }), 200
    
#     result = dict(upload_results[batch_id])
    
#     if result['success']:
#         response = {
#             "status": "completed",
#             "total_files": result['total_successful'] + result['total_failed'],
#             "successful_uploads": result['total_successful'], 
#             "failed_uploads": result['total_failed'],
#         }
        
#         if 'celery_task_id' in result:
#             response['celery_task_id'] = result['celery_task_id']
        
#         if result['failed_uploads']:
#             response['failed_files'] = result['failed_uploads']
    ...
    for post_item in bulk_posts_data:
        try:
            ...
            if 'youtube' in social_networks and 'tiktok' in social_networks:
                canal_data_tiktok = db.reference(f"canais/{canal_id_tiktok}", app=app1).get()
                canal_data_youtube = db.reference(f"canais/{canal_id_youtube}", app=app1).get()
...
def oauth2callback():
    ...
    try:
        ...
        canais_ref = db.reference("canais", app=app1).child(canal_id_replace)
        canais_ref.update({
            "id": canal_id_replace,
            "nome": canal_id,
            'TOKEN_ID': TOKEN_ID,
        ...
        return redirect("https://mediacutsstudio.com/new-post?auth_success=true")
        ...
        session.pop('canal_id', None)
        return redirect(
            "https://mediacutsstudio.com/new-post?"
...
def get_all_canais():
    canais_data = db.reference("canais", app=app1).get()
    if not canais_data:
        return jsonify({}), 200
    
    canais_list = [{**data, 'id': key} for key, data in canais_data.items()]
...
def get_all_shorts():
    shorts_data = db.reference("shorts", app=app1).get()
    if not shorts_data:
...
def get_short_details(short_id):
    short_data = db.reference(f"shorts/{short_id}", app=app1).get()
    if not short_data:
