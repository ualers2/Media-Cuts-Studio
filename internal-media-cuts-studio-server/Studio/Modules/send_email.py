...
def SendEmail(
    ...
    """
    email_type: "Failed Project", "Sucess Project", "Server Limitation", "Tiktok Publish Fail", "Youtube Publish Fail", "Sucess Created Account"
    """    
    ...
    with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as SMTP_server:
        ...
        try:    
            if email_type == "Failed Project":
                ...
                html_body = open(os.path.join(template_path, "FalhaDeProjeto.html"), encoding='utf-8').read().replace("{{erro}}", str(erro_project)).replace("{{title_origin}}", str(title_origin))


            elif email_type == "Monthly Limit Reached":
                corpo = f"""

                """
                MIME_server['Subject'] = "Limite de Videos por mes do plano atual Atingido"
                html_body = open(os.path.join(template_path, "email_Monthly_limitation.html"), encoding='utf-8').read().replace("{{current_usage}}", str(current_usage)).replace("{{username}}", str(username)).replace("{{plan_name}}", str(plan_name)).replace("{{plan_name}}", str(plan_name)).replace("{{reset_date}}", str(new_scheduled_time)).replace("{{plan_limit}}", str(plan_limit))


            elif email_type == "Plan Limitation":
                corpo = f"""

                """
                MIME_server['Subject'] = "Limite de Projetos em simultaneo do plano atual Atingido"
                html_body = open(os.path.join(template_path, "email_plan_limitation.html"), encoding='utf-8').read().replace("{{current_usage}}", str(current_usage)).replace("{{username}}", str(username)).replace("{{plan_name}}", str(plan_name)).replace("{{plan_name}}", str(plan_name)).replace("{{reset_date}}", str(new_scheduled_time)).replace("{{plan_limit}}", str(plan_limit))
                ...
                """
                MIME_server['Subject'] = "Sua conta foi atualizada para o Plano Content Creator 🚀"
